...
class DCT:
    ...
    def recomposeImage(self):
        ...
        if len(self.__block_list_) == 0:
            raise ValueError
        full_image = np.zeros(shape=(self.__height_, self.__width_), dtype=np.uint8)  # Builds empty image
    ...
    def lengthToBinary(self):
        ...
        msg_length = int(self.__message_length_ / 8)  # Decimal representation of the length
